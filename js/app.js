  ...
  $('.video-popup-trigger').magnificPopup({
    ...
    callbacks: {
      beforeOpen: function() {
        ...
        var src = $trigger.attr('data-video-src');
        var poster = $trigger.attr('data-video-poster');
        var title = $trigger.attr('data-video-title');
        var $player = $('#video-modal-player');
        if (poster) { $player.attr('poster', poster); }
        if (src) {
          $player.attr('src', src);
          $player[0].load();
          $player[0].play();
        }
        if (title) { $('#video-modal-caption').text(title); }
      },
      close: function() {
...
function getCurrentTheme(){
  // Portfolio default: dark, cinematic — until the visitor explicitly toggles it.
  let theme = 'dark';
